defmodule Doggo.Components do
  ...
  component(
    ...
    component_function: fn opts, extra ->
      ...
      quote do
        def unquote(name)(
              %{field: %Phoenix.HTML.FormField{} = field} = var!(assigns)
            ) do
          unquote(
            prepare_class(
              base_class,
        ...
        def unquote(name)(%{type: "checkbox"} = var!(assigns)) do
          var!(assigns) =
        ...
        def unquote(name)(%{type: "checkbox-group"} = var!(assigns)) do
          ~H"""
          <div class={@class ++ [Doggo.field_error_class(@errors)]}>
        ...
        def unquote(name)(%{type: "hidden", value: values} = var!(assigns))
        ...
        def unquote(name)(%{type: "hidden"} = var!(assigns)) do
          ~H"""
          <input type="hidden" name={@name} value={@value} />
          """
        end

        def unquote(name)(%{type: "radio-group"} = var!(assigns)) do
          ~H"""
          <div class={@class ++ [Doggo.field_error_class(@errors)]}>
        ...
        def unquote(name)(%{type: "select"} = var!(assigns)) do
          ~H"""
          <div class={@class ++ [Doggo.field_error_class(@errors)]}>
            <.label
              for={@id}
        ...
        def unquote(name)(%{type: "switch"} = var!(assigns)) do
          var!(assigns) =
            assign_new(var!(assigns), :checked, fn ->
              Phoenix.HTML.Form.normalize_value(
                "checkbox",
        ...
        def unquote(name)(%{type: "textarea"} = var!(assigns)) do
          ~H"""
          <div class={@class ++ [Doggo.field_error_class(@errors)]}>
            <.label
              for={@id}
